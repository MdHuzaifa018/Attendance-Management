...
/**
 * navItems — student sidebar links.
 * Students can only view their own data; no edit actions in nav.
 */
const navItems = [
...
/**
 * StudentLayout — shell for all /student/* pages.
 * Violet accent color for student role.
 */
const StudentLayout = () => {
          ...
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                ${
                  isActive
                    ? "text-violet-300 bg-violet-600/15 border-r-2 border-violet-500"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-slate-950 border-b border-slate-800 flex items-center gap-4 px-4 lg:px-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-violet-600/20 text-violet-300 border border-violet-500/30 px-2.5 py-1 rounded-full font-medium capitalize">
              {user?.role}
            </span>
          </div>
        </header>
