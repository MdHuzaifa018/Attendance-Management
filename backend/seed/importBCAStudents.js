...
// e.g. "1 MURLIDHAR YADAV UPENDRA YADAV 64 36 Poor"
// Or "25 RISHIKET KUMAR CHANDRA BHUSHAN PANDIT 5 3 Very Poor"
// It's tricky because Candidate Name and FatherName have variable words.
// But we know the end has 3 numbers and a remark, wait: No of Classes Attended (int), Percentage (int), Remark (string)
// Actually Remark can be "Very Poor", "Poor", "Good", "Excellent".

const parseLine = (line) => {
  const parts = line.split(" ");
  const rollNo = parts[0];
  
  // Let's find the numbers at the end.
  // Working backwards:
  // The remark can be 1 or 2 words (e.g. "Poor", "Very Poor")
  let remark = parts[parts.length - 1];
  ...
  const percentage = parts[ptr];
  ptr--;
  const classesAttended = parts[ptr];
  ptr--;

  // Now we have the names from index 1 to ptr.
  // We don't have a strict separator between Candidate Name and Father Name.
  // But usually names are 2-3 words each. Let's just split in half.
  const nameParts = parts.slice(1, ptr + 1);
  const mid = Math.ceil(nameParts.length / 2);
  const candidateName = nameParts.slice(0, mid).join(" ");
  const fatherName = nameParts.slice(mid).join(" ");
...
const run = async () => {
  try {
    ...
    for (const line of lines) {
      ...
      if (existingStudent) {
        // console.log(\`Skipping \${studentRoll}\`);
        continue;
      }
    ...
    console.log("✅ Successfully imported " + created + " students!");
  } catch (err) {
    console.error(err);
  } finally {
    process.exit(0);
